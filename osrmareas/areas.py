...
def make_grid(x, y, size_grid):
    ...
    pc = gpd.GeoSeries(coords_center, crs=crs_moll).to_crs(crs_lat)
    lat, lon = pc.x, pc.y
    destiny = list(np.array([lat, lon]).flatten())
    pc_list = pc.tolist()
    pol = Polygon(coords_pol)
    gdf = (
        gpd.GeoDataFrame({"geometry": pol}, index=[0]).set_crs(crs_moll).to_crs(crs_lat)
    )
...
def get_osrm_route(from_, to_, how="driving"):
    ...
    line = line.assign(
        ...
        time_driving_sec=duration,
        dist_eucl_km=ecl_dist(
            (to_[0], to_[1]), (from_[0], from_[1])
        ).km,  # euclidean distance
        dest_lon=to_[0],
        dest_lat=to_[1],
        origin_lon=from_[0],
        origin_lat=from_[1],
...
def get_routes(center_lat_long, radius_km=10, grid_km_size=1, filter_km=True):
    ...
    point_center = (
        gpd.GeoSeries(initial_center, crs=crs_lat)
        .to_crs(crs_moll)
    ...
    for final in tqdm(final_routes):
        ...
        all_routes_df = pd.concat([all_routes_df, get_osrm_route(center1, final1)])
